...
from third_party.reward_forcing.modeling.scheduler import SchedulerInterface, FlowMatchScheduler
from third_party.reward_forcing.modeling.tokenizers import HuggingfaceTokenizer
from third_party.reward_forcing.modeling.model import WanModel, RegisterTokens, GanAttentionBlock
from third_party.reward_forcing.modeling.vae import _video_vae
from third_party.reward_forcing.modeling.t5 import umt5_xxl
from third_party.reward_forcing.modeling.causal_model import CausalWanModel
...
class WanTextEncoder(torch.nn.Module):
    def __init__(self, model_name: str) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load(f"{model_name}/models_t5_umt5-xxl-enc-bf16.pth",
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name=f"{model_name}/google/umt5-xxl/", seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self, model_name: str):
        ...
        self.model = _video_vae(
            pretrained_path=f"{model_name}/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)

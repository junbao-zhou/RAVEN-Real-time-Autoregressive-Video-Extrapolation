...
class CausalInferencePipeline(torch.nn.Module):
        ...
        for window_index in range(window_num):
            ...
            with torch.no_grad():
                for block_idx in range(start_block, end_block + 1):
                    ...
                    block_timestep_indices = torch.nonzero(matches, as_tuple=True)[0]
                    if block_timestep_indices.numel() == 0:
                        raise ValueError(f"Cannot find timestep {block_time_step} in denoising_step_list.")
                    block_timestep_index = block_timestep_indices.item()
                    ...
                    denoised_pred_flat = denoised_pred.flatten(0, 1)
                    sampled_noise = torch.empty_like(denoised_pred_flat).normal_(generator=rng)
    ...
    def inference_rolling_forcing(self, *args, **kwargs):

...
class FSDPParameterFilter:
    ...
    def __call__(self, tensor: torch.Tensor):
        if tensor.untyped_storage().data_ptr() in self.model_parameters_storage:
            return False
...
class AsyncDoubleBufferGroupOffloadHandler(SynchronizedGroupOffloadHandler):
    ...
    def __init__(
        ...
        self.d2h_stream = torch.cuda.Stream(device=comm.get_device())
        self.h2d_stream = torch.cuda.Stream(device=comm.get_device())
    ...
    def synchronize_on_group_commit_forward(self, current_group):
        ...
        if current_group == 0:
            self.d2h_stream.wait_stream(torch.cuda.current_stream(device=comm.get_device()))
        ...
        if self.layer_window_map[self.offloaded_group_count] == current_group:
            ...
            self.d2h_stream.wait_stream(torch.cuda.current_stream(device=comm.get_device()))
            torch.cuda.current_stream(device=comm.get_device()).wait_stream(self.d2h_stream)
    ...
    def on_group_commit_backward(self):
        ...
        if self.layer_window_map[self.offloaded_group_count - 1] == self.current_group:
            # Stream synchronization both ways
            self.h2d_stream.wait_stream(torch.cuda.current_stream(device=comm.get_device()))
            torch.cuda.current_stream(device=comm.get_device()).wait_stream(self.h2d_stream)
...
class ActivationHandler:
    ...
    def _pack_kwargs(self, *args, **kwargs):
        ...
        return tuple(flat_args), spec

    def _unpack_kwargs(self, flat_args, spec):
        args, kwargs = pytree.tree_unflatten(flat_args, spec)
        return args, kwargs

    def _ckpt_forward(self, forward_method, *args, **kwargs):
        flat_args, spec = self._pack_kwargs(*args, **kwargs)

        def my_function(*inputs):
            nonlocal forward_method, spec
            unpacked_args, unpacked_kwargs = self._unpack_kwargs(inputs, spec)
...
def enable_activation_offloading(model, strategy="fsdp", enable_ckpt=False):
    """
